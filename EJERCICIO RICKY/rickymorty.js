...
const printinDocuments = (list) => {
  for (const item of list) {
    container.innerHTML += `
        ...
        <p>${item.origin}</p>
        <p>${item.location}</p>
        <img class="frame"src=${item.image} alt=${item.name}/>
        </div>`;
  }
};

const mapCharacter = () => {
  const newCharacters = characters.results.map((character) => ({
    name: character.name,
  ...
  printinDocuments(newCharacters);
};
